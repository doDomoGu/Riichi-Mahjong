...
APP_DIR="${APP_DIR:-/opt/riichi-mahjong}"
...
BUNDLE="$(mktemp -t riichi-mahjong-deploy.XXXXXX).tgz"
...
expect "$PASS_SCP" -- -P "$ECS_PORT" \
  ...
  "$BUNDLE" "${SSH_TARGET}:/tmp/riichi-mahjong-deploy.tgz"
...
expect "$PASS_SCP" -- -P "$ECS_PORT" \
  ...
  -o PreferredAuthentications=password \
  -o PubkeyAuthentication=no \
  deploy/remote-deploy.sh "${SSH_TARGET}:/tmp/riichi-mahjong-remote-deploy.sh"
...
expect "$PASS_SSH" -- -p "$ECS_PORT" \
  ...
  -o PreferredAuthentications=password \
  -o PubkeyAuthentication=no \
  "$SSH_TARGET" "bash /tmp/riichi-mahjong-remote-deploy.sh"

echo ""
echo "部署完成。访问："
echo "  http://dodomogu.com/mahjong/"
echo "  http://dodomogu.com/mahjong/wind/"
echo "  http://dodomogu.com/mahjong/score-practice/"
